...
export default function Kasse() {
 ...
 const payOrders = async (orderIds) => {
  ...
  const startCleaning = (g) => {
    if (!g.isTable) return;
    const delayMin = tenant?.table_release_delay_minutes ?? 5;
    if (delayMin <= 0) return;
    setCleaning((prev) => {
      const filtered = prev.filter((c) => c.key !== g.key);
      return [...filtered, { key: g.key, label: g.label, until: Date.now() + delayMin * 60000 }];
    });
  };
